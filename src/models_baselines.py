...
def choi_big(x, is_training, config):
    print('Input: ' + str(x.get_shape))
    input_layer = tf.expand_dims(x, 3)
    bn_input = tf.layers.batch_normalization(input_layer, training=is_training)
    ...
    pool1 = tf.layers.max_pooling2d(inputs=bn_conv1, pool_size=[4, 1], strides=[4, 1])
    ...
    pool2 = tf.layers.max_pooling2d(inputs=bn_conv2, pool_size=[1, 4], strides=[1, 4])
...
def choi_medium(x, is_training, config):
    print('Input: ' + str(x.get_shape))
    input_layer = tf.expand_dims(x, 3)
    bn_input = tf.layers.batch_normalization(input_layer, training=is_training)
...
def timbre(x, is_training, config, num_filt=1):
    ...
    conv2 = tf.layers.conv2d(inputs=input_pad_5,
                             filters=3*num_filt,
    ...
    conv3 = tf.layers.conv2d(inputs=input_pad_3, 
                             filters=6*num_filt,
    ...
    conv5 = tf.layers.conv2d(inputs=input_pad_7,
                             filters=5*num_filt,
    ...
    conv6 = tf.layers.conv2d(inputs=input_pad_5, 
                             filters=5*num_filt,
    ...
    conv7 = tf.layers.conv2d(inputs=input_pad_3, 
                             filters=10*num_filt,
    ...
    # BACK END
    conv2 = tf.layers.conv2d(inputs=out_front_end, 
    	                     filters=32, 
